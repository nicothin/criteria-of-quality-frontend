...
Страница без стилей и скриптов, будучи открыта в браузере, выглядит логично и работает. Например:
...
### 1.8. Отсутствуют закомментированные блоки

**Исключения:** особые состояния блоков (пример: блок авторизации для авторизованного посетителя).


### 1.9. Для контентных изображений прописаны атрибуты размеров


### 1.10. Для ссылок прописаны правильные протоколы

Для ссылок на адрес электропочты — `mailto:`, для телефонов — `tel:`.
...
### 2.11. Контентные изображения не должны работать как «распорки»
...
### 2.12. У отдельных блоков не фиксированы размеры
...
### 2.13. В коде отсутствуют неиспользуемые в разметке селекторы
...
### 4.2. Выбраны подходящие форматы для изображений
...
### 4.3. Шрифты подключены в нужных форматах

Необходимые форматы: `woff`, `woff2`.


### 4.4. Растровые изображения оптимизированы

Растровые изображения должны быть оптимизированы, но не иметь видимых отличий от макета.


### 4.5. Векторные изображения не содержат растра
...
- Задан общие стиль для контентного текста.
